...
import { Briefcase, DollarSign, ListChecks, PlusCircle, Eye, MessageSquare, Heart } from 'lucide-react';
import type { Account, DailyAccountSummary as AIDailySummary, Project } from '@/types';
import { getProjectsForAccount } from '@/lib/data';
...
export default function AccountCard({ account }: AccountCardProps) {
  ...
  useEffect(() => {
    setProjects(getProjectsForAccount(account.id));
  }, [account.id]);
  ...
  const fetchDailySummary = async () => {
    ...
    try {
      const summary = await generateDailyAccountSummary({
        ...
        recentUpdates: "Placeholder: Recent updates indicate active engagement.", // Replace with actual data
        keyMetrics: "Placeholder: Key metrics are trending positively.", // Replace with actual data
      });
    ...
    <Card className="shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col h-full">
      ...
      <CardContent className="flex-grow space-y-3">
        ...
        {account.status === 'Active' && (
          <div className="pt-2 border-t mt-3">
            <h4 className="text-xs font-semibold uppercase text-muted-foreground mb-1">AI Daily Summary</h4>
            {isLoadingSummary ? (
      ...
      <CardFooter className="flex justify-between items-center pt-4">
        <Button variant="outline" size="sm" asChild>
          <Link href={`/accounts?id=${account.id}#details`}> {/* Simplified link for now */}
