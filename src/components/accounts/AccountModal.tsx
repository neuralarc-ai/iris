...
import { Mail, Phone, Briefcase, Globe, Tag, Pencil, Users, FileText, MessageSquareHeart, Lightbulb, MapPin, UserCheck, Activity, Building2, Target, BrainCircuit, RefreshCw, CopyIcon, SendIcon, Inbox, AtSign, Shield, Computer } from 'lucide-react';
...
export default function AccountModal({ accountId, open, onClose }: AccountModalProps) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      setAccount(acc);
      // Fetch assigned user name/email if owner_id exists
      if (acc?.owner_id) {
  ...
  useEffect(() => {
    if (account?.owner_id) setAssignedUserId(account.owner_id);
  }, [account]);
...
\nHi ${account?.contact_person_name},
...
\nBest regards,\n${currentUser?.name || '[Your Name]'}\n${userCompany.name}\n${currentUser?.email || '[Your Email]'}\n${userCompany.website}`);
  ...
  const getMailClientUrl = (client: string) => {
    ...
    const to = encodeURIComponent(account?.contact_email || '');
    switch (client) {
      case 'gmail':
      ...
      default:
        return `mailto:${account?.contact_email}?subject=${subject}&body=${body}`;
    }
  };
              ...
              <div className="flex items-center gap-4">
                <Avatar className="h-16 w-16 bg-[#5E6156]">
                  <AvatarFallback className="text-2xl bg-[#2B2521] font-bold text-white">
                    {account?.contact_person_name?.split(' ').map(n => n[0]).join('') || account?.name?.[0] || '?'}
                  </AvatarFallback>
                </Avatar>
                <div className="flex flex-col gap-0.5">
                  <div className="flex items-center gap-2">
                    <DialogTitle className="text-3xl font-bold text-[#282828]">{account?.contact_person_name || account?.name}</DialogTitle>
                  </div>
                  <p className="text-lg text-[#5E6156] leading-tight">{account?.job_title || 'Role/Title'}</p>
                  <p className="text-lg text-[#5E6156] leading-tight">{account?.name || 'Company'}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Badge className={`text-xs mr-6 ${account?.status === 'Active' ? 'bg-green-300 text-green-900' : 'bg-gray-300 text-gray-700'}`}>{account?.status}</Badge>
              </div>
            </div>
          </DialogHeader>
        </div>
                      ...
                      {account?.contact_person_name && (
                        <div className="bg-[#F8F7F3] p-3 rounded-md flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-base text-[#282828] font-medium">{account.contact_person_name}</p>
                          </div>
                        </div>
                      )}
                      {account?.contact_email && (
                        <div className="bg-[#F8F7F3] p-3 rounded-md flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-base text-[#282828] font-medium break-all">{account.contact_email}</p>
                          </div>
                        </div>
                      )}
                      {account?.contact_phone && (
                        <div className="bg-[#F8F7F3] p-3 rounded-md flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-base text-[#282828] font-medium">{account.contact_phone}</p>
                          </div>
                        </div>
                      )}
                      {account?.job_title && (
                        <div className="bg-[#F8F7F3] p-3 rounded-md flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-base text-[#282828] font-medium">{account.job_title}</p>
                          </div>
                        </div>
                      )}
                    </div>
                      ...
                      {account?.country && (
                        <div className="bg-[#F8F7F3] p-3 rounded-md flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-base font-medium text-[#282828]">{account.country}</p>
                          </div>
                        </div>
                      )}
                      {account?.description && (
                      ...
                      {ownerName && (
                        <div className="bg-[#F8F7F3] p-3 rounded-md flex items-start gap-3">
                          ...
                          <div className="w-full">
                            <p className="text-sm text-[#5E6156]">Assigned User</p>
                            ...
                            {ownerName && (
                              <div className="mt-2 text-sm text-[#998876]">Currently assigned to: <span className="font-medium text-[#282828]">{ownerName}</span></div>
                            )}
                          </div>
                        </div>
                      )}
